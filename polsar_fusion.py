...
def pca_cov(IM, FS):
    ...
    C = C - np.mean(C, 0)
    C = C / np.std(C, 0)
    COVAR = np.dot(C.T, C) / (nrows*ncols)
    p, vectors=np.linalg.eig(COVAR)
    ## finds the probabilities covered by the eigenvectors
    #p=values[:]*1.0/np.sum(values[:])
    ## finds the fusion points based on the probability
    aux=np.zeros([nrows,ncols])
...
def fusao(IM, metodo, NUM_RAIOS, comp_princ):
    nrows, ncols, nc = IM.shape
    FS=np.zeros([nrows,ncols])
    MCOVAR = np.zeros([nc, nc])
    if metodo==1:
    ...
    if metodo==2:
        ...
        #FS=pca(IM, FS)
        FS, comp_princ, MCOVAR = pca_cov(IM, FS)
    if metodo==3:
        print("Finding fusion using E-ROC")
    ...
    if metodo==7:
        print("Finding threshold fusion using E-ROC")
        FS = fusion_threshold(IM, nrows, ncols, nc, comp_princ, NUM_RAIOS)
    #if metodo==7:
        #print("finding fusion using Majority")
        #FS=majority(IM, FS)
    return FS, comp_princ, MCOVAR
#
## Shows the evidence
...
def show_fusion_evidence_simulated(pauli, nrows, ncols, FUSION, img_rt):
	...
	plt.imshow(PIA)
	plt.show()
def fusion_threshold(IM, nrows, ncols, nc, coef_pca, NUM_RAIOS):
    thr = 1.0
    ind_fusion = []
    c = []
    #for i in range(nc):
    ...
    d = 4
    c = [0, 1, 2, 3]
    IMAUX = np.zeros([nrows, ncols, d])
    for i in range(d):
        IMAUX[:, :, i] = IM[:, :, c[i]]
    FS = np.zeros([nrows, ncols])
    FS = roc(IMAUX, FS, NUM_RAIOS)
    return FS
