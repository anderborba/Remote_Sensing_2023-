...
def loglik_intensity_prod(x, z, Ni, Nf):
    ...
    for k in range(Ni, Nf):
        soma1 = soma1 + np.log(iv(0, 2 * rho * L * z[k] / ((1 - rho**2) * h)))
        soma2 = soma2 + np.log(kv(L - 1, 2 * L * z[k] /   ((1 - rho**2) * h)))
    #
    aux1 = (L + 1) * np.log(L)
    aux2 = L * sum(z[Ni: Nf]) / (Nf + 1 - Ni)
    aux3 = np.log(math.gamma(L))
    aux4 = np.log(1 - rho**2)
    aux5 = 0.5 * (L + 1) * np.log(mu1)
    aux6 = 0.5 * (L + 1) * np.log(mu2)
    aux7 = soma1 / (Nf + 1 - Ni)
    aux8 = soma2 / (Nf + 1 - Ni)
    #aux8 = 0
    ####Beware! The signal is negative because BFGS routine finds the point of minimum
...
# output: Log-likelihood function value
#
def loglik_intensity_prod_sum(x, z, Ni, Nf):
    L   = x[0]
    rho = np.abs(x[1])
    mu1 = x[2]
    mu2 = x[3]
    ...
    for k in range(Ni, Nf):
        ...
        #print(k)
        #print(z[k])
        soma = soma + pp.log_pdf_prod_intensities(z[k], L, rho, mu1, mu2)
        #print(soma)
    #
    ####Beware! The signal is negative because BFGS routine finds the point of minimum
    #print(soma)
